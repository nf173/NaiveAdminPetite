<template>
  <div class="tabs-view">
    <n-scrollbar x-scrollable>
      <draggable class="tags" item-key="fullPath" animation="200" :list="tabsList">
        <template #item="{ element }">
          <n-el tag="div" class="tag-card" :class="element.name === route.name ? 'active' : ''"
            @click.stop="tabClick(element.name)" @contextmenu="handleContextMenu($event, element)">
            <n-ellipsis style="max-width: 110px">
              {{ element.meta?.title }}
            </n-ellipsis>
            <SvgIcon name="close" v-if="!element.meta?.affix" @click.stop="closeTab(element)" />
          </n-el>
        </template>
      </draggable>
    </n-scrollbar>

    <n-dropdown placement="bottom-start" :show="showDropdown" :x="dropdownX" :y="dropdownY" :options="tabsMenuOptions"
      @clickoutside="handleClickOutside" @select="handleSelect" />
  </div>
</template>
...
<script setup>
  ...
  */
  const closeAll = () => {
    ...
    router.replace(settingStore.baseHome);
  };
